...
for (const dataset of datasets) {
    ...
    for (let i = 0; i < tests_per_dataset; i += 1) {
        const indices = Array.from({length: 9}, _ => Math.round(Math.random() * 7))
        ...
        query += ` & ! (${cells.genes[indices[7]]} = 0 & 0 = ${cells.genes[indices[8]]})`
        let ctx = {}
        ...
        const q_filter = mkFilterBuilder(query)[0](ctx)
        const m_filter = e =>
            ...
            ! (((ctx[cells.genes[indices[7]]][e] || 0) === 0) && ((ctx[cells.genes[indices[8]]][e] || 0) === 0))
        it(`testing query ${query} on dataset ${dataset}`, () => expect(barcodes.filter(q_filter)).toEqual(barcodes.filter(m_filter)))
    }
}